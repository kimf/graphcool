...
export default async (event: FunctionEvent<EventData>) => {
  ...
  try {
    ...
    // create new user
    const userId = await createGraphcoolUser(api, email, hash, firstName, lastName)
...
async function createGraphcoolUser(api: GraphQLClient, email: string, password: string, firstName: string, lastName: string): Promise<string> {
  const mutation = `
